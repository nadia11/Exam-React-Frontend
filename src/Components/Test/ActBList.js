...
function ActBList() {
  ...
  const getActTests = async () => {
    let url = `${process.env.REACT_APP_BASE_URL}getallactstests?batch_id=${batch_id}`;
    try {
  ...
  const handleSubmitAssign = async (e) => {
    ...
    if (tests.length === 0) {
      ...
      try {
        const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}delete_batch_acttest`, dataSend, {
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-body">
                <div class="row">
                  <div class="col-6">
                    <h4 class="card-title">Assign Act Tests From Batch</h4>
                  </div> 
                  {data.length > 0 ? (
