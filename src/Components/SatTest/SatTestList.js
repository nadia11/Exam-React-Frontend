...
function SatTestList() {
  ...
  const unassignTest = async (testId) => { 
    if (window.confirm("Are you sure you want to Unassign Test?")) {
      const response = await axios.delete(
        `${process.env.REACT_APP_BASE_URL}unassignsattest/${testId}/${id}`
      );
  ...
  const gettests = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}getactivestests/${id}`
    );
  ...
  const handleSubmitAssign = async (e) => {
    e.preventDefault();
    if (tests.length === 0) {
      toast.error("Please Choose SAT Test");
    }
    else {
      let dataSend = {
        batch_id: id,
      ...
      try {
        const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}assignsattests`, dataSend, {
          withCredentials: true,
        });

        if (data) {
          toast.success("SAT Test Assigned Successfully.");
          window.location.reload(true);
        }
      } catch (error) {
        console.log(error);
